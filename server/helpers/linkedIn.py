...
def get_company_by_id(id: str) -> Company:
    ...
    return Company(name= company_extended['name'] if 'name' in company_extended else "",
                   description=company_extended['description'] if 'description' in company_extended else "", 
                   tagLine=company_extended['tagline'] if 'tagline' in company_extended else "", 
                   companyUrl=company_extended['companyPageUrl'] if 'companyPageUrl' in company_extended else "", 
                   headquarter=company_extended['headquarter'] if 'headquarter' in company_extended else None,
                   phoneNumber=company_extended['phone']['number'] if (('phone' in company_extended) and ('number' in company_extended['phone'])) else "",
                   specialities=company_extended['specialities']) if 'specialities' in company_extended else []
    
def get_job_lite(job: dict[str, any]) -> Job :
    return Job(position=job['title'],
               company=get_company_by_id(get_company_id(job)),
               applyUrl=job['applyMethod']['companyApplyUrl'] if (('applyMethod' in job) and ('companyApplyUrl' in job['applyMethod'])) else "",
               )
